...
if($titulo == ""){
    ...
    echo "<script>updateSlideFailed()</script>";
    echo "<h1><center>Desculpe, houve algum erro</center></h1>";
} else {
    $sql = "INSERT INTO carousel (titulo , video , imagem) VALUES ('$titulo','$video','$imagem')";
    $exc = mysqli_query($conexao , $sql);
    echo "<script>updateSlideSuccess()</script>";
...
function uploadImage($files, $titulo){
    ...
    $imgSize = $files['size'];
    if(!empty($imgFile)){
        $upload_dir = '../../../uploadCarousel/'; // diretório de uploadp
